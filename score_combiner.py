...
class GuessScoreInfo:
    def __init__(self, guess, score_stats_dict):
        self._guess = guess
    ...
    def incorporate_score(self, type, score):
        self._type_dict[type] = ScoreInfo(type, score, self._score_stats_dict[type])
...
class ScoreInfo:
    ...
    def __init__(self, type, score, score_stats):
        self._type = type
        self._score = score
...
class ScoreCombiner:
        ...
        for type, score_dict in entry_score_distributions.iteritems():
            ...
            for guess, score in score_dict.iteritems():
                if not guess in guesses:
                    guesses[guess] = GuessScoreInfo(guess, self._score_stats)
    ...
    def sorted_guess_list(self, guesses):
        return sorted([(guess, guess_score_info.combined_score()) for guess, guess_score_info in guesses.iteritems()],
